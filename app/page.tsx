...
type Player={name:string;position:string;team:string;fantasy:number;stats:string;logo?:string};
...
function GameCard({game:g}:{game:Game}){const live=/IN_PROGRESS|HALFTIME/i.test(g.status);return <article className="card"><div className="gamehead"><span className={live?"status liveText":"status"}>{g.status.replaceAll("_"," ")}</span><span>{g.clock&&g.period?g.clock+" • Q"+g.period:""}</span></div><div className="teams"><div><b>{g.away.abbr}</b><span>{g.away.name}</span></div><strong>{g.away.score}</strong></div><div className="teams"><div><b>{g.home.abbr}</b><span>{g.home.name}</span></div><strong>{g.home.score}</strong></div><div className="players"><div className="ptable"><span>PLAYER</span><span>POS</span><span>PPR</span></div>{g.players.map((p,i)=><div className="prow" key={p.name+p.team+i}><div><b className="player-name">{p.logo&&<img src={p.logo} alt="" />}{p.name}</b><small>{p.team} • {p.stats}</small></div><span>{p.position}</span><strong>{fmt(p.fantasy)}</strong></div>)}{!g.players.length&&<div className="noPlayers">Player stats appear when the feed provides box scores.</div>}</div></article>}
